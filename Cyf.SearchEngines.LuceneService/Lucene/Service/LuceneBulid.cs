...
namespace Cyf.SearchEngines.LuceneService.Service
{
    public class LuceneBulid : ILuceneBulid
    {
        ...
        public void BuildIndex(List<User> ciList, string pathSuffix = "", bool isCreate = false)
        {
        ...
        /// </summary>
        /// <param name="ci"></param>
        public void InsertIndex(User ci)
        {
            IndexWriter writer = null;
            try
            {
        ...
        public void InsertIndexMuti(List<User> ciList)
        {
        ...
        /// </summary>
        /// <param name="ciList"></param>
        public void DeleteIndexMuti(List<User> ciList)
        {
        ...
        /// </summary>
        /// <param name="ci"></param>
        public void DeleteIndex(User ci)
        {
            IndexReader reader = null;
            try
            {
        ...
        /// </summary>
        /// <param name="ci"></param>
        public void UpdateIndex(User ci)
        {
            IndexWriter writer = null;
            try
            {
                ...
                writer.MergeFactor = 100;//控制多个segment合并的频率，默认10
                writer.UseCompoundFile = true;//创建符合文件 减少索引文件数量
                writer.UpdateDocument(new Term("id", ci.id.ToString()), ParseCItoDoc(ci));
            }
            catch (Exception ex)
            {
                logger.Error("InsertIndex异常", ex);
        ...
        /// </summary>
        /// <param name="ciList">sourceflag统一的</param>
        public void UpdateIndexMuti(List<User> ciList)
        {
            IndexWriter writer = null;
            try
            {
        ...
        private void CreateCIIndex(IndexWriter writer, User ci)
        {
            try
            {
        ...
        /// <param name="ci"></param>
        /// <returns></returns>
        private Document ParseCItoDoc(User ci)
        {
            Document doc = new Document();

            doc.Add(new Field("id", ci.id.ToString(), Field.Store.YES, Field.Index.NOT_ANALYZED));
            doc.Add(new Field("name", ci.name, Field.Store.YES, Field.Index.ANALYZED));//盘古分词
            doc.Add(new Field("company_id", ci.company_id.ToString(), Field.Store.YES, Field.Index.NOT_ANALYZED));
            doc.Add(new Field("position", ci.position.ToString(), Field.Store.YES, Field.Index.NOT_ANALYZED));
