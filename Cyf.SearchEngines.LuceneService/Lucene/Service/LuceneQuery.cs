...
namespace Cyf.SearchEngines.LuceneService.Service
{
    public class LuceneQuery : ILuceneQuery
    {
        ...
        /// <returns></returns>
        public List<User> QueryIndex(string queryString)
        {
            IndexSearcher searcher = null;
            try
            {
                List<User> ciList = new List<User>();
        ...
        /// <returns></returns>
        public List<User> QueryIndexPage(string queryString, int pageIndex, int pageSize, out int totalCount, string priceFilter, string priceOrderBy)
        {
            totalCount = 0;
            IndexSearcher searcher = null;
            try
        ...
        #region private
        private User DocumentToCommodityInfo(Document doc)
        {
            return new User()
                       {
                           ...
                           company_id = int.Parse(doc.Get("company_id")),
